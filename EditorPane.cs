...
namespace VsLink
{
	public sealed class EditorPane : WindowPane, IOleCommandTarget, IVsPersistDocData, IPersistFileFormat
	{
		public EditorPane()
			: base(null)
		{
			FileInfo f = new FileInfo(Assembly.GetExecutingAssembly().Location);
			string path = Path.Combine(f.Directory.FullName, "ScintillaNET.dll");
		...
		private const string fileExtension = ".vslink";
		private const uint fileFormat = 0;

		// Object that handles the editor window.
		private LinkEditor editorControl;
					...
					case (uint)VSConstants.VSStd97CmdID.Cut:
						{
							...
							if (editorControl.SelectedText.Length > 0)
							{
		...
		int IPersistFileFormat.InitNew(uint nFormatIndex)
		{
			...
			{
				throw new ArgumentException("ExceptionMessageFormat");
			}
		...
		int IVsPersistDocData.SaveDocData(VSSAVEFLAGS dwSave, out string pbstrMkDocumentNew, out int pfSaveCanceled)
		{
			...
			switch (dwSave)
			{
				...
				case VSSAVEFLAGS.VSSAVE_SilentSave:
					{
						...
						switch ((tagVSQuerySaveResult)result)
						{
							...
							default:
								throw new COMException("ExceptionMessageQEQS");
						}
						break;
					}
				...
				default:
					throw new ArgumentException("ExceptionMessageSaveFlag");
			};
